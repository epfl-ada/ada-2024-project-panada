...
def generate_clean_df_channels_en(data_path):
    """
    Load and clean the 'df_channels_en' dataset.
    
    Steps:
    1. Load the dataset and select relevant columns.
    2. Remove rows with missing values.
    3. Handle duplicate 'channel' entries:
       - If duplicate channels have the same 'category_cc', keep one entry.
       - If duplicate channels have different 'category_cc', drop all entries for those channels.
    4. Return the cleaned DataFrame.
    
    :param data_path: Path to the folder containing the dataset.
...
#monetization = pd.read_csv("youtube_monetization_api.csv")

def generate_clean_monetization(data_path):
    """
    Load and clean the 'youtube_monetization_api' dataset.
    
    Steps:
    1. Load the dataset and remove rows with missing values.
    2. Handle duplicate 'channel_id' entries:
       - If duplicate 'channel_id' values have the same associated data, keep one entry.
       - If duplicate 'channel_id' values have conflicting data, drop all entries for those 'channel_id'.
...
def generate_clean_channel_monetization(data_path):
    """
    Generate a cleaned and merged dataset combining 'channel_id' and 'channel' information.
    
    Steps:
    1. Generate cleaned datasets for channels and monetization.
    2. Merge the datasets on 'channel_id' and 'channel'.
    3. Drop the redundant 'channel' column after the merge.
    4. Write the merged dataset to a CSV file named 'clean_channel_monetization.csv'.

    :param data_path: Path to the folder containing the datasets.
    :return: None
    """
